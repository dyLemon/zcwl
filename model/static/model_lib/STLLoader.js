...
THREE.STLLoader.prototype = Object.assign( Object.create( THREE.Loader.prototype ), {
	...
	parse: function ( data ) {
		...
		function UnZip(buffer) {
		    var unzip_buffer = new Uint8Array(buffer.length);
		    unzip_buffer = pako.inflate(buffer);
		    return unzip_buffer.buffer;
		}

		// start
		var unzipData = UnZip(data);
		var binData = ensureBinary(unzipData);

		return isBinary(binData) ? parseBinary(binData) : parseASCII(ensureString(unzipData));
